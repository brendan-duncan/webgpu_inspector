/**
 * Drive an ablation sweep for one captured draw (tier 3b).
 *
 * shader_ablation.js rewrites a shader so its entry point can be stopped short
 * at a run-time cut point; draw_timing.js knows how to replay a draw with
 * timestamp queries. This module joins them: replay one draw N+1 times with the
 * cut advancing, and difference the results into a per-statement cost.
 *
 * The one piece neither module already had is injecting the ablation uniform's
 * bind group into a replayed draw. With an "auto" pipeline layout that is free
...
/**
 * Measure the per-statement GPU cost of one draw's shader stage.
 *
 * @param {Object} params
 ...
 * @param {"vertex"|"fragment"} [params.stage="fragment"]
...
export async function measureStatementCosts({
    ...
    if (stage !== "vertex" && stage !== "fragment") {
        return { ok: false, reason: `Ablation supports vertex and fragment stages, not "${stage}".`, notes: [] };
    }

    const replay = new CaptureReplay(device, database);
    try {
        // --- locate the draw and its pass ---------------------------------
        const pass = collectPasses(commands).find((p) => p.commands.includes(drawCommand));
        if (!pass) {
            return { ok: false, reason: "The draw was not found inside a render pass.", notes: [] };
        }
        if (pass.kind !== "render") {
            return { ok: false, reason: "Only draws in render passes can be ablated.", notes: [] };
        }
        ...
        if (!plan) {
            return { ok: false, reason: "The draw could not be reconstructed from the capture.", notes: [] };
        }
        if (plan.pipelineId === null || plan.pipelineId === undefined) {
            return { ok: false, reason: "The draw has no resolvable pipeline.", notes: [] };
        }

        const pipelineObj = replay.database.getObject(plan.pipelineId);
        const desc = pipelineObj?.descriptor;
        if (!desc?.vertex || !desc?.fragment?.module) {
            return { ok: false, reason: "The draw's pipeline was not fully captured (it needs both stages).", notes: [] };
        }
        ...
        if (!code) {
            return { ok: false, reason: `The ${stage} shader source was not captured.`, notes: [] };
        }

        // An explicit pipeline layout may declare more bind groups than the
        // shader actually reads, and the ablation group must not collide with
        // any of them.
        ...
        if (attachments.error) {
            return { ok: false, reason: attachments.error, notes: Array.from(replay.notes) };
        }
        ...
        if (!instrumentedModule) {
            return { ok: false, reason: "The instrumented shader failed to compile.", notes: Array.from(replay.notes) };
        }
        ...
        if (!otherModule) {
            return { ok: false, reason: "The draw's other shader stage was not captured.", notes: Array.from(replay.notes) };
        }
        ...
        if (stage === "vertex") {
            ...
            if (!stubModule) {
                return { ok: false, reason: "The stub fragment shader failed to compile.", notes: Array.from(replay.notes) };
            }
        ...
        const ablationBGL = device.createBindGroupLayout({
            entries: [{
                ...
                visibility: stage === "vertex" ? 1 : 2,  // VERTEX : FRAGMENT
                buffer: { type: "uniform" },
            }],
        });
        ...
        let layoutBGLs = null;
        if (explicitLayout) {
            // bindGroupLayouts is positional, so any gap between the original
            // groups and the ablation group has to be filled with empty ones.
            const bindGroupLayouts = explicitLayout.bindGroupLayouts.slice();
            while (bindGroupLayouts.length < instrumented.group) {
                bindGroupLayouts.push(device.createBindGroupLayout({ entries: [] }));
            }
            bindGroupLayouts[instrumented.group] = ablationBGL;
            const built = await validated(replay, "The ablation pipeline layout could not be created", () =>
                device.createPipelineLayout({ bindGroupLayouts }));
            if (!built) {
                return { ok: false, reason: "The pipeline layout could not be extended with the ablation group.", notes: Array.from(replay.notes) };
            }
            layout = built;
            layoutBGLs = bindGroupLayouts;
        }
        ...
        if (!gpuPipeline) {
            return { ok: false, reason: "The ablation pipeline could not be created.", notes: Array.from(replay.notes) };
        }
        ...
        for (let g = 0; g < plan.bindGroups.length; ++g) {
            ...
            if (info.error) {
                return { ok: false, reason: `A bind group could not be replayed: ${info.error}`, notes: Array.from(replay.notes) };
            }
        ...
        for (const vb of plan.vertexBuffers) {
            if (vb && !replay.getBuffer(vb.bufferId)) {
                return { ok: false, reason: "A vertex buffer could not be re-created.", notes: Array.from(replay.notes) };
            }
        }
        ...
        const ablationBindGroup = device.createBindGroup({
            layout: layout === "auto"
                ? gpuPipeline.getBindGroupLayout(instrumented.group)
                : ablationBGL,
            entries: [{ binding: instrumented.binding, resource: { buffer: cutBuffer } }],
        });
        bindGroups.push({ index: instrumented.group, bindGroup: ablationBindGroup, dynamicOffsets: null });
        ...
        async function timeOnce(repeats) {
            ...
            try {
                ...
                // setRenderState binds everything in item.bindGroups, which
                // already includes the ablation group appended above.
        ...
        // Probe the uncut shader to size the repeat count: a draw costing less
        // than the timestamp quantum has to be repeated or every difference in
        // the sweep is noise.
        const probe = await measureCut(instrumented.fullCut, 1);
        if (probe === null) {
            return { ok: false, reason: "The draw could not be timed.", notes: Array.from(replay.notes) };
        }
        ...
        if (repeats > 1) {
            notes.push(`Each cut point was measured with the draw encoded ${repeats} times and the total divided, because a single execution is below the GPU's timestamp granularity.`);
        }
        ...
        notes.push("The draw is replayed in isolation against freshly cleared attachments, so depth contents from earlier passes are absent.");
