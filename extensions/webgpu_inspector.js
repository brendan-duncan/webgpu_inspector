...
class ObjectsPanel {
    constructor(database) {
        ...
        // Object lists
        this.uiPendingRenderPipelines = this._createObjectListUI(this.debugPanel, "Pending Async Render Pipelines");
        this.uiRenderPipelines = this._createObjectListUI(this.debugPanel, "Render Pipelines");
        this.uiComputePipelines = this._createObjectListUI(this.debugPanel, "Compute Pipelines");
        this.uiShaderModules = this._createObjectListUI(this.debugPanel, "Shader Modules");
        ...
        this.uiBindGroups = this._createObjectListUI(this.debugPanel, "BindGroups");
        this.uiBindGroupLayouts = this._createObjectListUI(this.debugPanel, "BindGroupLayouts");
    }
    ...
    updateLabels() {
        this.uiPendingRenderPipelinesStat.innerHTML = this.database.pendingRenderPipelines.size.toLocaleString("en-US");
        this.uiRenderPipelinesStat.innerHTML = this.database.renderPipelines.size.toLocaleString("en-US");
        this.uiShaderModulesStat.innerHTML = this.database.shaderModules.size.toLocaleString("en-US");
        this.uiBindGroupsStat.innerHTML = this.database.bindGroups.size.toLocaleString("en-US");
        this.uiTexturesStat.innerHTML = this.database.textures.size.toLocaleString("en-US");
        this.uiSamplersStat.innerHTML = this.database.samplers.size.toLocaleString("en-US");

        let size = 0;
        for (const buffer of this.database.buffers.values()) {
            size += buffer.size;
        }
        this.uiBuffersStat.innerHTML = `${this.database.buffers.size.toLocaleString("en-US")} Size: ${size.toLocaleString("en-US")} Bytes`;
    }
    ...
    addObject(id, object, pending) {
        ...
        if (object instanceof Sampler) {
            ...
            o = document.createElement('li');
            o.innerHTML = `ShaderModule ${id} Size:${object.descriptor[0].code.length.toLocaleString("en-US")} bytes`;
            this.uiShaderModules.appendChild(o);
            object.element = o;
        } else if (object instanceof RenderPipeline) {
            o = document.createElement('li');
            o.innerHTML = `RenderPipeline ${id} time:${object.time?.toLocaleString("en-US") ?? '0'}ms`;
            object.element = o;
            if (pending)
                this.uiPendingRenderPipelines.appendChild(o);
            else
                this.uiRenderPipelines.appendChild(o);
    ...
    resolvePendingObject(id, object) {
        const vs = this.database.getObject(object.descriptor[0].vertex.module.__id);
        const fs = this.database.getObject(object.descriptor[0].fragment.module.__id);

        object.element.innerHTML = `RenderPipeline ${id} time:${object.time.toLocaleString("en-US")}ms vs_size:${vs.size.toLocaleString("en-US")} fs_size:${fs.size.toLocaleString("en-US")}`;
    ...
    updateFrameStats() {
        ...
        this.uiFrameRenderPasses.innerHTML = this.database.renderPassCount.toLocaleString("en-US");
    }
}
...
class WebGPUInspector {
    ...
    _objectHasMethods(object) {
        for (const m in object) {
            if (typeof(object[m]) == "function" && WebGPUInspector._skipMethods.indexOf(m) == -1) {
                return true;
            }
        }
        return false;
...
window.addEventListener("message", function(event) {
    if (event.source == window) {
        ...
        if (event.data.action == "enable_webgpu_inspector") {
            webgpuInspector.enable();
        } else if (event.data.action == "disable_webgpu_inspector") {
            webgpuInspector.disable();
        }
    }
});
