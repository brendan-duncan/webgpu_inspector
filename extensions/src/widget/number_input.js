...
export class NumberInput extends Span {
  constructor(parent, options) {
    ...
    if (value === null || value === undefined) value = 0;
      ...
      isExpr = isNaN(value);
      if (!isExpr) value = parseFloat(value);
    ...
    if (options.full) container.classList.add('full');

    if (this.disabled) container.classList.add('disabled');
    ...
    input.addEventListener('keydown', function (e) {
      if (e.keyCode == 38) innerInc(1, e);
      else if (e.keyCode == 40) innerInc(-1, e);
      else return;
      e.stopPropagation();
      e.preventDefault();
      return true;
    ...
    if (this.disabled) dragger.classList.add('disabled');
    ...
    function innerDown(e) {
      ...
      if (!self.disabled) {
        if (self.element.requestPointerLock) self.element.requestPointerLock();
    ...
    function innerInc(v, e) {
      ...
      if (self.max !== null && value > self.max) value = self.max;

      if (self.min !== null && value < self.min) value = self.min;

      value = value.toFixed(self.precision);
      if (self.units) value += self.units;
      input.value = value;
  ...
  setValue(v, skipEvent) {
    ...
    if (!isExpr) {
      ...
      if (this.units) v += this.units;
    }
    if (this.input.value != v) {
      this.input.value = v;
      if (!skipEvent) this.input.onChange.emit(v);
    }
  }
