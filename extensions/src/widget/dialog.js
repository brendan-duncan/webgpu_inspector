...
export class Dialog extends Div {
  constructor(options) {
    ...
    if (!options?.noCloseButton) {
      ...
      closeButton.addEventListener('mouseup', function (e) {
        if (e.target === closeButton.element) Widget.window.overlay = null;
      });
    }
    ...
    if (!options?.parent) Widget.window.overlay = background;
  ...
  close() {
    Widget.window.overlay = null;
  }
}

Dialog.isDialog = true;
Dialog._idPrefix = 'DIALOG';
